...
class NoteDatabase{
  ...
  Future<List<Note>> readAllNote() async{
    final db = await instance.database;
    const orderBy = '${NoteFields.prioritise} ASC, ${NoteFields.lastModifyTime} DESC';
    final result = await db.query(
      databaseName,
      orderBy: orderBy
    );
    List<Note> notesList = result.isNotEmpty ? result.map((e) => Note.fromJson(e)).toList() : [];
    return notesList;
  }
  ...
  Future<List<Note>> searchNote (String searchString) async{
    ...
    /*List<Map<String, dynamic>> result = await db.rawQuery(
       'SELECT * FROM ${databaseName} WHERE title=?', [searchString]
    );*/
    ...
    await db.rawQuery(
        'SELECT * FROM $databaseName WHERE $databaseName'
            ' MATCH "{title content} : $searchString" ORDER BY bm25($databaseName, 10.0, 5.0)'
    );*/
